with
  ...
  main_drug_period as (
    select
      ...
      min_by(unit_per_hour_active_ingredient, start_time) as first_drug_rate,
      min_by(unit_active_ingredient, start_time) as unit,
...
where
    icu_stay_id in (
        select icu_stay_id
        from `medicu-production.research_vasoactive_drugs_short_term_responses_2025.01_eligibility_criteria`
    )
