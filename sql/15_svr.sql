-- Values of SVR from 30 minutes before to 120 minutes after initiation of a vasoactive drug.
...
with
  ...
  , create_svr as (
    select
      v.icu_stay_id as icu_stay_id, 
      v.time as time, 
      systemic_vascular_resistance as svr_measured,
      case
        when cardiac_output = 0
        then null
        else cast(round((invasive_mbp - cvp)*80/cardiac_output) as int64)
      end as svr_calculated,
    from medicu-beta.latest_one_icu.vital_measurements v
    inner join medicu-beta.latest_one_icu.advanced_hemodynamic_monitoring a
    on v.icu_stay_id = a.icu_stay_id
    and v.time = a.time
  )
  -- Obtain SVR values
  , gain_svr as (
    select
      ...
      coalesce(svr_measured, svr_calculated) as svr,
    from main_drug_true_period
    inner join create_svr using (icu_stay_id)
