...
for (drug in names(gamma_values_list)) {
  ...
  # ===============================
  preds <- predict(gam_cvp, newdata = base_grid, type = "link")
  ...
  pred_df <- base_grid %>%
    mutate(
      ...
      cvp_diff = fit_diff,
      invasive_cvp = cvp_time0_value + cvp_diff
    )
...
for (drug in names(gamma_values_list)) {
  ...
  model_vars <- c("cvp_diff", "offset", "gamma", "cvp_time0_value",
                   "cvp_lpf_offset_minus15", "cvp_lpf_offset_minus10", "cvp_lpf_offset_minus5",
...
y_range <- diff(range(plot_df$invasive_cvp, na.rm = TRUE))
...
cvp_spline_combined <- ggplot(
  plot_df,
  aes(x = offset, y = invasive_cvp, color = active_ingredient_name)
) +
  ...
  labs(
    ...
    title = "Predicted CVP Trajectory"
  ) +
...
print(cvp_spline_combined)  

ggsave(
  paste0(output_dir, "fig_cvp_compare_drug_", output_ver, ".png"),
  cvp_spline_combined,
