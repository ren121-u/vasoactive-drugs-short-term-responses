...
for (drug in names(gamma_values_list)) {
  ...
  # ===============================
  preds <- predict(gam_hr, newdata = base_grid, type = "link")
  ...
  pred_df <- base_grid %>%
    mutate(
      ...
      hr_diff = fit_diff,
      invasive_hr = hr_time0_value + hr_diff
    )
...
for (drug in names(gamma_values_list)) {
  ...
  model_vars <- c("hr_diff", "offset", "gamma", "hr_time0_value",
                   "hr_lpf_offset_minus15", "hr_lpf_offset_minus10", "hr_lpf_offset_minus5",
...
y_range <- diff(range(plot_df$invasive_hr, na.rm = TRUE))
...
hr_spline_combined <- ggplot(
  plot_df,
  aes(x = offset, y = invasive_hr, color = active_ingredient_name)
) +
...
print(hr_spline_combined)  

ggsave(
  paste0(output_dir, "fig_hr_compare_drug_", output_ver, ".png"),
  hr_spline_combined,
