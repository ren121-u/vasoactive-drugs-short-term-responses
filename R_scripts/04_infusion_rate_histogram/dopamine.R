...
# Cases with MAP, dopamine only
df_filtered <- df_base %>%
  filter(active_ingredient_name == "dopamine") %>%
  semi_join(df_map %>%
              filter(active_ingredient_name == "dopamine") %>%
...
p <- ggplot(df_filtered, aes(x = gamma)) +
  ...
  labs(
    title = "Distribution of Dopamine Starting Dose",
    x = "Dose (μg/kg/min)",
    y = "Count"
  ) +
  theme_classic() +
...
# Save
ggsave(
  filename = paste0(output_dir, "gamma_hist_dopamine_", output_ver, ".png"),
  plot = p,
