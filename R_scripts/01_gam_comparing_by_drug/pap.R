...
for (drug in names(gamma_values_list)) {
  ...
  # ===============================
  preds <- predict(gam_pap, newdata = base_grid, type = "link")
  ...
  pred_df <- base_grid %>%
    mutate(
      ...
      pap_diff = fit_diff,
      invasive_pap = pap_time0_value + pap_diff
    )
...
for (drug in names(gamma_values_list)) {
  ...
  model_vars <- c("pap_diff", "offset", "gamma", "pap_time0_value",
                   "pap_lpf_offset_minus15", "pap_lpf_offset_minus10", "pap_lpf_offset_minus5",
...
y_range <- diff(range(plot_df$invasive_pap, na.rm = TRUE))
...
pap_spline_combined <- ggplot(
  plot_df,
  aes(x = offset, y = invasive_pap, color = active_ingredient_name)
) +
  ...
  labs(
    ...
    title = "Predicted mean PAP Trajectory"
  ) +
...
print(pap_spline_combined)  

ggsave(
  paste0(output_dir, "fig_pap_compare_drug_", output_ver, ".png"),
  pap_spline_combined,
