...
for (drug in names(gamma_values_list)) {
  ...
  # ===============================
  preds <- predict(gam_co, newdata = base_grid, type = "link")
  ...
  pred_df <- base_grid %>%
    mutate(
      ...
      co_diff = fit_diff,
      invasive_co = co_time0_value + co_diff
    )
...
for (drug in names(gamma_values_list)) {
  ...
  model_vars <- c("co_diff", "offset", "gamma", "co_time0_value",
                   "co_lpf_offset_minus15", "co_lpf_offset_minus10", "co_lpf_offset_minus5",
...
y_range <- diff(range(plot_df$invasive_co, na.rm = TRUE))
...
co_spline_combined <- ggplot(
  plot_df,
  aes(x = offset, y = invasive_co, color = active_ingredient_name)
) +
...
print(co_spline_combined)  

ggsave(
  paste0(output_dir, "fig_co_compare_drug_", output_ver, ".png"),
  co_spline_combined,
