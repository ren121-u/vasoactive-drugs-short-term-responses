...
for (drug in names(gamma_values_list)) {
  ...
  # ===============================
  preds <- predict(gam_map, newdata = base_grid, type = "link")
  ...
  pred_df <- base_grid %>%
    mutate(
      ...
      map_diff = fit_diff,
      invasive_map = map_time0_value + map_diff
    )
...
for (drug in names(gamma_values_list)) {
  ...
  model_vars <- c("map_diff", "offset", "gamma", "map_time0_value",
                   "map_lpf_offset_minus15", "map_lpf_offset_minus10", "map_lpf_offset_minus5",
...
y_range <- diff(range(plot_df$invasive_map, na.rm = TRUE))
...
map_spline_combined <- ggplot(
  plot_df,
  aes(x = offset, y = invasive_map, color = active_ingredient_name)
) +
  ...
  labs(
    ...
    title = "Predicted MAP Trajectory"
  ) +
...
print(map_spline_combined)  

ggsave(
  paste0(output_dir, "fig_map_compare_drug_", output_ver, ".png"),
  map_spline_combined,
