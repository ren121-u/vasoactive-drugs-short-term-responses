...
for (drug in names(gamma_values_list)) {
  ...
  # ===============================
  preds <- predict(gam_dbp, newdata = base_grid, type = "link")
  ...
  pred_df <- base_grid %>%
    mutate(
      ...
      dbp_diff = fit_diff,
      invasive_dbp = dbp_time0_value + dbp_diff
    )
...
for (drug in names(gamma_values_list)) {
  ...
  model_vars <- c("dbp_diff", "offset", "gamma", "dbp_time0_value",
                   "dbp_lpf_offset_minus15", "dbp_lpf_offset_minus10", "dbp_lpf_offset_minus5",
...
y_range <- diff(range(plot_df$invasive_dbp, na.rm = TRUE))
...
dbp_spline_combined <- ggplot(
  plot_df,
  aes(x = offset, y = invasive_dbp, color = active_ingredient_name)
) +
  ...
  labs(
    ...
    title = "Predicted DBP Trajectory"
  ) +
...
print(dbp_spline_combined)  

ggsave(
  paste0(output_dir, "fig_dbp_compare_drug_", output_ver, ".png"),
  dbp_spline_combined,
