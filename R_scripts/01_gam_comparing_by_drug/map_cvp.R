...
for (drug in names(gamma_values_list)) {
  ...
  # ===============================
  preds <- predict(gam_map_cvp, newdata = base_grid, type = "link")
  ...
  pred_df <- base_grid %>%
    mutate(
      ...
      invasive_map_cvp = map_cvp_time0_value + map_cvp_diff
    )
...
for (drug in names(gamma_values_list)) {
  ...
  model_vars <- c("map_cvp_diff", "offset", "gamma", "map_cvp_time0_value",
                   "map_cvp_lpf_offset_minus15", "map_cvp_lpf_offset_minus10", "map_cvp_lpf_offset_minus5",
...
y_range <- diff(range(plot_df$invasive_map_cvp, na.rm = TRUE))
...
map_cvp_spline_combined <- ggplot(
  plot_df,
  aes(x = offset, y = invasive_map_cvp, color = active_ingredient_name)
) +
  ...
  labs(
    ...
    title = "Predicted mPP Trajectory"
  ) +
...
print(map_cvp_spline_combined)  

ggsave(
  paste0(output_dir, "fig_map_cvp_compare_drug_", output_ver, ".png"),
  map_cvp_spline_combined,
