...
# -------------------------------
# Create the CO value at time 0 for each drug and ICU stay
# -------------------------------
...
df_post <- complete_df %>%
  ...
  inner_join(co_time0_df, by = c("icu_stay_id", "active_ingredient_name")) %>%
  mutate(
...
for (drug in names(gamma_values_list)) {
  ...
  model_vars <- c(
    "co_diff", "offset", "gamma", "co_time0_value",
    "co_lpf_offset_minus15",
    "co_lpf_offset_minus10",
    "co_lpf_offset_minus5",
  ...
  # --- GAM (difference + six pre-initiation CO values) ---
  gam_co <- gam(
    co_diff ~
      s(offset) +
      gamma +
  ...
  # ===============================
  co_pre_median <- co_pre_wide %>%
    filter(active_ingredient_name == drug) %>%
    summarise(across(starts_with("co_lpf_offset_"),
                     ~ median(., na.rm = TRUE)))

  co_time0_median <- co_time0_df %>%
    filter(active_ingredient_name == drug) %>%
    summarise(co_time0_value = median(co_time0_value, na.rm = TRUE))
  ...
  base_grid <- current_df %>%
    ...
    bind_cols(co_pre_median) %>%
    bind_cols(co_time0_median) %>%
    slice(rep(1, 136)) %>%
    mutate(
  ...
  for (g in gamma_values_list[[drug]]) {
    ...
    preds <- predict(gam_co, newdata = pred_grid, type = "link")
    ...
    pred_list[[as.character(g)]] <-
      ...
      mutate(
        co_diff = fit_diff,
        predicted_co = co_time0_value + co_diff,
        gamma_value = g
      )
  }
  ...
  y_range <- diff(range(plot_df$predicted_co, na.rm = TRUE))
  ...
  label_df <- gam_ready %>%
    ...
    mutate(
      gamma_label = paste0(gamma_group, ": n=", n_patients),
      x_pos = 45,
      y_pos = seq(3, by = 0.15, length.out = n())
    )
  ...
  labs(
    title = paste("Predicted CO Trajectory (", drug_title_map[[drug]], ")", sep = ""),
    x = "Time from catecholamine start (min)",
    y = "Predicted CO (L/min)",
...
print(p)
  ...
  ggsave(
    filename = paste0(output_dir, "fig_co_compare_dose_", drug, "_", output_ver, ".png"),
    plot = p,
