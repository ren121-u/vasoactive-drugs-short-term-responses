...
# --- Load data ---
df <- read.csv(paste0(data_dir, data_ver_pre, '_dbp_filtered.csv'))
...
# ===============================
# Reshape DBP at offsets -15 to 0 into wide format
# ===============================
dbp_pre_wide <- complete_df %>%
  filter(offset >= -15 & offset <= 0) %>%
  select(icu_stay_id, active_ingredient_name, offset, dbp_lpf) %>%
  group_by(icu_stay_id, active_ingredient_name, offset) %>%
  summarise(dbp_lpf = median(dbp_lpf, na.rm = TRUE), .groups = "drop") %>%
  pivot_wider(
    names_from  = offset,
    values_from = dbp_lpf,
    names_prefix = "dbp_lpf_offset_"
  ) %>%
  rename_with(~ gsub("-", "minus", .x))

# -------------------------------
# Create the DBP value at time 0 for each drug and ICU stay
# -------------------------------
dbp_time0_df <- complete_df %>%
  filter(offset == 0) %>%
  select(icu_stay_id, active_ingredient_name, dbp_lpf) %>%
  rename(dbp_time0_value = dbp_lpf)
...
df_post <- complete_df %>%
  ...
  inner_join(dbp_time0_df, by = c("icu_stay_id", "active_ingredient_name")) %>%
  mutate(
    dbp_diff = dbp_lpf - dbp_time0_value
  ) %>%
  inner_join(
    dbp_pre_wide %>% select(-dbp_lpf_offset_0),
...
for (drug in names(gamma_values_list)) {
  ...
  model_vars <- c(
    "dbp_diff", "offset", "gamma", "dbp_time0_value",
    "dbp_lpf_offset_minus15",
    "dbp_lpf_offset_minus10",
    "dbp_lpf_offset_minus5",
  ...
  # --- GAM (difference + six pre-initiation DBP values) ---
  gam_dbp <- gam(
    dbp_diff ~
      s(offset) +
      gamma +
  ...
  # ===============================
  dbp_pre_median <- dbp_pre_wide %>%
    filter(active_ingredient_name == drug) %>%
    summarise(across(starts_with("dbp_lpf_offset_"),
                     ~ median(., na.rm = TRUE)))

  dbp_time0_median <- dbp_time0_df %>%
    filter(active_ingredient_name == drug) %>%
    summarise(dbp_time0_value = median(dbp_time0_value, na.rm = TRUE))
  ...
  base_grid <- current_df %>%
    ...
    bind_cols(dbp_pre_median) %>%
    bind_cols(dbp_time0_median) %>%
    slice(rep(1, 136)) %>%
    mutate(
  ...
  for (g in gamma_values_list[[drug]]) {
    ...
    preds <- predict(gam_dbp, newdata = pred_grid, type = "link")
    ...
    pred_list[[as.character(g)]] <-
      ...
      mutate(
        dbp_diff = fit_diff,
        predicted_dbp = dbp_time0_value + dbp_diff,
        gamma_value = g
      )
  }
  ...
  y_range <- diff(range(plot_df$predicted_dbp, na.rm = TRUE))
  ...
  label_df <- gam_ready %>%
    ...
    mutate(
      gamma_label = paste0(gamma_group, ": n=", n_patients),
      x_pos = 45,
      y_pos = seq(40, by = 1.5, length.out = n())
    )
  ...
  labs(
    title = paste("Predicted DBP Trajectory (", drug_title_map[[drug]], ")", sep = ""),
    x = "Time from catecholamine start (min)",
    y = "Predicted DBP (mmHg)",
...
print(p)
  ...
  ggsave(
    filename = paste0(output_dir, "fig_dbp_compare_dose_", drug, "_", output_ver, ".png"),
    plot = p,
