...
# --- Load data ---
df <- read.csv(paste0(data_dir, data_ver_pre, '_hr_filtered.csv'))
...
# ===============================
# Reshape HR at offsets -15 to 0 into wide format
# ===============================
hr_pre_wide <- complete_df %>%
  filter(offset >= -15 & offset <= 0) %>%
  select(icu_stay_id, active_ingredient_name, offset, hr_lpf) %>%
  group_by(icu_stay_id, active_ingredient_name, offset) %>%
  summarise(hr_lpf = median(hr_lpf, na.rm = TRUE), .groups = "drop") %>%
  pivot_wider(
    names_from  = offset,
    values_from = hr_lpf,
    names_prefix = "hr_lpf_offset_"
  ) %>%
  rename_with(~ gsub("-", "minus", .x))

# -------------------------------
# Create the HR value at time 0 for each drug and ICU stay
# -------------------------------
hr_time0_df <- complete_df %>%
  filter(offset == 0) %>%
  select(icu_stay_id, active_ingredient_name, hr_lpf) %>%
  rename(hr_time0_value = hr_lpf)
...
df_post <- complete_df %>%
  ...
  inner_join(hr_time0_df, by = c("icu_stay_id", "active_ingredient_name")) %>%
  mutate(
    hr_diff = hr_lpf - hr_time0_value
  ) %>%
  inner_join(
    hr_pre_wide %>% select(-hr_lpf_offset_0),
...
for (drug in names(gamma_values_list)) {
  ...
  model_vars <- c(
    "hr_diff", "offset", "gamma", "hr_time0_value",
    "hr_lpf_offset_minus15",
    "hr_lpf_offset_minus10",
    "hr_lpf_offset_minus5",
  ...
  # --- GAM (difference + six pre-initiation HR values) ---
  gam_hr <- gam(
    hr_diff ~
      s(offset) +
      gamma +
  ...
  # ===============================
  hr_pre_median <- hr_pre_wide %>%
    filter(active_ingredient_name == drug) %>%
    summarise(across(starts_with("hr_lpf_offset_"),
                     ~ median(., na.rm = TRUE)))

  hr_time0_median <- hr_time0_df %>%
    filter(active_ingredient_name == drug) %>%
    summarise(hr_time0_value = median(hr_time0_value, na.rm = TRUE))
  ...
  base_grid <- current_df %>%
    ...
    bind_cols(hr_pre_median) %>%
    bind_cols(hr_time0_median) %>%
    slice(rep(1, 136)) %>%
    mutate(
  ...
  for (g in gamma_values_list[[drug]]) {
    ...
    preds <- predict(gam_hr, newdata = pred_grid, type = "link")
    ...
    pred_list[[as.character(g)]] <-
      ...
      mutate(
        hr_diff = fit_diff,
        predicted_hr = hr_time0_value + hr_diff,
        gamma_value = g
      )
  }
  ...
  y_range <- diff(range(plot_df$predicted_hr, na.rm = TRUE))
  ...
  label_df <- gam_ready %>%
    ...
    mutate(
      gamma_label = paste0(gamma_group, ": n=", n_patients),
      x_pos = 45,
      y_pos = seq(95, by = 1.5, length.out = n())
    )
  ...
  labs(
    title = paste("Predicted HR Trajectory (", drug_title_map[[drug]], ")", sep = ""),
...
print(p)
  ...
  ggsave(
    filename = paste0(output_dir, "fig_hr_compare_dose_", drug, "_", output_ver, ".png"),
    plot = p,
