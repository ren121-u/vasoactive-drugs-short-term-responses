...
# --- Load data ---
df <- read.csv(paste0(data_dir, data_ver_pre, '_map_cvp_filtered.csv'))
...
# ===============================
# Reshape MAP-CVP at offsets -15 to 0 into wide format
# ===============================
map_cvp_pre_wide <- complete_df %>%
  filter(offset >= -15 & offset <= 0) %>%
  select(icu_stay_id, active_ingredient_name, offset, map_cvp_lpf) %>%
  group_by(icu_stay_id, active_ingredient_name, offset) %>%
  summarise(map_cvp_lpf = median(map_cvp_lpf, na.rm = TRUE), .groups = "drop") %>%
  pivot_wider(
    names_from  = offset,
    values_from = map_cvp_lpf,
    names_prefix = "map_cvp_lpf_offset_"
  ) %>%
  rename_with(~ gsub("-", "minus", .x))

# -------------------------------
# Create the MAP-CVP value at time 0 for each drug and ICU stay
# -------------------------------
map_cvp_time0_df <- complete_df %>%
  filter(offset == 0) %>%
  select(icu_stay_id, active_ingredient_name, map_cvp_lpf) %>%
  rename(map_cvp_time0_value = map_cvp_lpf)
...
df_post <- complete_df %>%
  ...
  inner_join(map_cvp_time0_df, by = c("icu_stay_id", "active_ingredient_name")) %>%
  mutate(
    map_cvp_diff = map_cvp_lpf - map_cvp_time0_value
  ) %>%
  inner_join(
    map_cvp_pre_wide %>% select(-map_cvp_lpf_offset_0),
...
for (drug in names(gamma_values_list)) {
  ...
  model_vars <- c(
    "map_cvp_diff", "offset", "gamma", "map_cvp_time0_value",
    "map_cvp_lpf_offset_minus15",
    "map_cvp_lpf_offset_minus10",
    "map_cvp_lpf_offset_minus5",
  ...
  # --- GAM (difference + six pre-initiation MAP-CVP values) ---
  gam_map_cvp <- gam(
    map_cvp_diff ~
      s(offset) +
      gamma +
      ti(gamma, offset) +
      s(map_cvp_time0_value) +
      map_cvp_lpf_offset_minus15 +
      map_cvp_lpf_offset_minus10 +
      map_cvp_lpf_offset_minus5 +
      female + age + body_weight +
  ...
  # ===============================
  map_cvp_pre_median <- map_cvp_pre_wide %>%
    filter(active_ingredient_name == drug) %>%
    summarise(across(starts_with("map_cvp_lpf_offset_"),
                     ~ median(., na.rm = TRUE)))

  map_cvp_time0_median <- map_cvp_time0_df %>%
    filter(active_ingredient_name == drug) %>%
    summarise(map_cvp_time0_value = median(map_cvp_time0_value, na.rm = TRUE))
  ...
  base_grid <- current_df %>%
    ...
    bind_cols(map_cvp_pre_median) %>%
    bind_cols(map_cvp_time0_median) %>%
    slice(rep(1, 136)) %>%
    mutate(
  ...
  for (g in gamma_values_list[[drug]]) {
    ...
    preds <- predict(gam_map_cvp, newdata = pred_grid, type = "link")
    ...
    pred_list[[as.character(g)]] <-
      pred_grid %>%
      mutate(
        map_cvp_diff = fit_diff,
        predicted_map_cvp = map_cvp_time0_value + map_cvp_diff,
        gamma_value = g
      )
  }
  ...
  y_range <- diff(range(plot_df$predicted_map_cvp, na.rm = TRUE))
  ...
  label_df <- gam_ready %>%
    ...
    mutate(
      gamma_label = paste0(gamma_group, ": n=", n_patients),
      x_pos = 45,
      y_pos = seq(45, by = 1.5, length.out = n())
    )
  ...
  labs(
    title = paste("Predicted mPP Trajectory (", drug_title_map[[drug]], ")", sep = ""),
    x = "Time from catecholamine start (min)",
    y = "Predicted mPP (mmHg)",
...
print(p)
  ...
  ggsave(
    filename = paste0(output_dir, "fig_map_cvp_compare_dose_", drug, "_", output_ver, ".png"),
    plot = p,
