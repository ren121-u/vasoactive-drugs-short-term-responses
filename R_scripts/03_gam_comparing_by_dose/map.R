...
# ===============================
# Reshape MAP at offsets -15 to 0 into wide format
# ===============================
...
# -------------------------------
# Create the MAP value at time 0 for each drug and ICU stay
# -------------------------------
...
df_post <- complete_df %>%
  ...
  inner_join(map_time0_df, by = c("icu_stay_id", "active_ingredient_name")) %>%
  mutate(
    map_diff = map_lpf - map_time0_value
  ) %>%
  inner_join(
...
for (drug in names(gamma_values_list)) {
  ...
  # --- GAM (difference + six pre-initiation MAP values) ---
  gam_map <- gam(
    map_diff ~
      s(offset) +
      gamma +
  ...
  # ===============================
  map_pre_median <- map_pre_wide %>%
    filter(active_ingredient_name == drug) %>%
    summarise(across(starts_with("map_lpf_offset_"),
                     ~ median(., na.rm = TRUE)))

  map_time0_median <- map_time0_df %>%
    filter(active_ingredient_name == drug) %>%
    summarise(map_time0_value = median(map_time0_value, na.rm = TRUE))
  ...
  base_grid <- current_df %>%
    ...
    bind_cols(map_pre_median) %>%
    bind_cols(map_time0_median) %>%
    slice(rep(1, 136)) %>%
    mutate(
  ...
  for (g in gamma_values_list[[drug]]) {
    ...
    preds <- predict(gam_map, newdata = pred_grid, type = "link")
    ...
    pred_list[[as.character(g)]] <-
      ...
      mutate(
        map_diff = fit_diff,
        predicted_map = map_time0_value + map_diff,
        gamma_value = g
      )
  }
  ...
  y_range <- diff(range(plot_df$predicted_map, na.rm = TRUE))
  ...
  label_df <- gam_ready %>%
    ...
    mutate(
      gamma_label = paste0(gamma_group, ": n=", n_patients),
      x_pos = 45,
      y_pos = seq(55, by = 1.5, length.out = n())
    )
  ...
  labs(
    title = paste("Predicted MAP Trajectory (", drug_title_map[[drug]], ")", sep = ""),
    x = "Time from catecholamine start (min)",
    y = "Predicted MAP (mmHg)",
...
print(p)
  ...
  ggsave(
    filename = paste0(output_dir, "fig_map_compare_dose_", drug, "_", output_ver, ".png"),
    plot = p,
