...
# --- Load data ---
df <- read.csv(paste0(data_dir, data_ver_pre, '_pap_filtered.csv'))
...
# ===============================
# Reshape PAP at offsets -15 to 0 into wide format
# ===============================
pap_pre_wide <- complete_df %>%
  filter(offset >= -15 & offset <= 0) %>%
  select(icu_stay_id, active_ingredient_name, offset, pap_lpf) %>%
  group_by(icu_stay_id, active_ingredient_name, offset) %>%
  summarise(pap_lpf = median(pap_lpf, na.rm = TRUE), .groups = "drop") %>%
  pivot_wider(
    names_from  = offset,
    values_from = pap_lpf,
    names_prefix = "pap_lpf_offset_"
  ) %>%
  rename_with(~ gsub("-", "minus", .x))

# -------------------------------
# Create the PAP value at time 0 for each drug and ICU stay
# -------------------------------
pap_time0_df <- complete_df %>%
  filter(offset == 0) %>%
  select(icu_stay_id, active_ingredient_name, pap_lpf) %>%
  rename(pap_time0_value = pap_lpf)
...
df_post <- complete_df %>%
  ...
  inner_join(pap_time0_df, by = c("icu_stay_id", "active_ingredient_name")) %>%
  mutate(
    pap_diff = pap_lpf - pap_time0_value
  ) %>%
  inner_join(
    pap_pre_wide %>% select(-pap_lpf_offset_0),
...
for (drug in names(gamma_values_list)) {
  ...
  model_vars <- c(
    "pap_diff", "offset", "gamma", "pap_time0_value",
    "pap_lpf_offset_minus15",
    "pap_lpf_offset_minus10",
    "pap_lpf_offset_minus5",
  ...
  # --- GAM (difference + six pre-initiation PAP values) ---
  gam_pap <- gam(
    pap_diff ~
      s(offset) +
      gamma +
  ...
  # ===============================
  pap_pre_median <- pap_pre_wide %>%
    filter(active_ingredient_name == drug) %>%
    summarise(across(starts_with("pap_lpf_offset_"),
                     ~ median(., na.rm = TRUE)))

  pap_time0_median <- pap_time0_df %>%
    filter(active_ingredient_name == drug) %>%
    summarise(pap_time0_value = median(pap_time0_value, na.rm = TRUE))
  ...
  base_grid <- current_df %>%
    ...
    bind_cols(pap_pre_median) %>%
    bind_cols(pap_time0_median) %>%
    slice(rep(1, 136)) %>%
    mutate(
  ...
  for (g in gamma_values_list[[drug]]) {
    ...
    preds <- predict(gam_pap, newdata = pred_grid, type = "link")
    ...
    pred_list[[as.character(g)]] <-
      ...
      mutate(
        pap_diff = fit_diff,
        predicted_pap = pap_time0_value + pap_diff,
        gamma_value = g
      )
  }
  ...
  y_range <- diff(range(plot_df$predicted_pap, na.rm = TRUE))
  ...
  label_df <- gam_ready %>%
    ...
    mutate(
      gamma_label = paste0(gamma_group, ": n=", n_patients),
      x_pos = 45,
      y_pos = seq(17, by = 0.6, length.out = n())
    )
  ...
  scale_y_continuous(
    limits = c(17, 25),
    breaks = seq(17, 25, by = 2)
  ) +

  labs(
    title = paste("Predicted mean PAP Trajectory (", drug_title_map[[drug]], ")", sep = ""),
    x = "Time from catecholamine start (min)",
    y = "Predicted mean PAP (mmHg)",
...
print(p)
  ...
  ggsave(
    filename = paste0(output_dir, "fig_pap_compare_dose_", drug, "_", output_ver, ".png"),
    plot = p,
