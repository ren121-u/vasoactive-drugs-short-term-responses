...
# --- Load data ---
df <- read.csv(paste0(data_dir, data_ver_pre, '_cvp_filtered.csv'))
...
# ===============================
# Reshape CVP at offsets -15 to 0 into wide format
# ===============================
cvp_pre_wide <- complete_df %>%
  filter(offset >= -15 & offset <= 0) %>%
  select(icu_stay_id, active_ingredient_name, offset, cvp_lpf) %>%
  group_by(icu_stay_id, active_ingredient_name, offset) %>%
  summarise(cvp_lpf = median(cvp_lpf, na.rm = TRUE), .groups = "drop") %>%
  pivot_wider(
    names_from  = offset,
    values_from = cvp_lpf,
    names_prefix = "cvp_lpf_offset_"
  ) %>%
  rename_with(~ gsub("-", "minus", .x))

# -------------------------------
# Create the CVP value at time 0 for each drug and ICU stay
# -------------------------------
cvp_time0_df <- complete_df %>%
  filter(offset == 0) %>%
  select(icu_stay_id, active_ingredient_name, cvp_lpf) %>%
  rename(cvp_time0_value = cvp_lpf)
...
df_post <- complete_df %>%
  ...
  inner_join(cvp_time0_df, by = c("icu_stay_id", "active_ingredient_name")) %>%
  mutate(
    cvp_diff = cvp_lpf - cvp_time0_value
  ) %>%
  inner_join(
    cvp_pre_wide %>% select(-cvp_lpf_offset_0),
...
for (drug in names(gamma_values_list)) {
  ...
  model_vars <- c(
    "cvp_diff", "offset", "gamma", "cvp_time0_value",
    "cvp_lpf_offset_minus15",
    "cvp_lpf_offset_minus10",
    "cvp_lpf_offset_minus5",
  ...
  # --- GAM (difference + six pre-initiation CVP values) ---
  gam_cvp <- gam(
    cvp_diff ~
      s(offset) +
      gamma +
  ...
  # ===============================
  cvp_pre_median <- cvp_pre_wide %>%
    filter(active_ingredient_name == drug) %>%
    summarise(across(starts_with("cvp_lpf_offset_"),
                     ~ median(., na.rm = TRUE)))

  cvp_time0_median <- cvp_time0_df %>%
    filter(active_ingredient_name == drug) %>%
    summarise(cvp_time0_value = median(cvp_time0_value, na.rm = TRUE))
  ...
  base_grid <- current_df %>%
    ...
    bind_cols(cvp_pre_median) %>%
    bind_cols(cvp_time0_median) %>%
    slice(rep(1, 136)) %>%
    mutate(
  ...
  for (g in gamma_values_list[[drug]]) {
    ...
    preds <- predict(gam_cvp, newdata = pred_grid, type = "link")
    ...
    pred_list[[as.character(g)]] <-
      ...
      mutate(
        cvp_diff = fit_diff,
        predicted_cvp = cvp_time0_value + cvp_diff,
        gamma_value = g
      )
  }
  ...
  y_range <- diff(range(plot_df$predicted_cvp, na.rm = TRUE))
  ...
  label_df <- gam_ready %>%
    ...
    mutate(
      gamma_label = paste0(gamma_group, ": n=", n_patients),
      x_pos = 45,
      y_pos = seq(6, by = 0.6, length.out = n())
    )
  ...
  labs(
    title = paste("Predicted CVP Trajectory (", drug_title_map[[drug]], ")", sep = ""),
    x = "Time from catecholamine start (min)",
    y = "Predicted CVP (mmHg)",
...
print(p)
  ...
  ggsave(
    filename = paste0(output_dir, "fig_cvp_compare_dose_", drug, "_", output_ver, ".png"),
    plot = p,
